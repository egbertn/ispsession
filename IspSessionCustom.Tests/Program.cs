...
public class Program
{
    public static void Main(string[] args)
    {
        ...
        app.MapGet("/counter",async (HttpContext context) =>
        {
            //juse for demonstration purpose, sesssionid could come from anywhere
            var success = context.Request.Query.TryGetValue("sessionId", out var sessionIds);
            if (!success)
            {
                return Results.BadRequest("We need query string parameter named Sessionid");
            }
            await using var sessionState = await context.GetCustomSessionId(sessionIds);
        ...
        app.MapGet("/counterWithApp",async (HttpContext context, IApplicationState appState) =>
        {
            var success = context.Request.Query.TryGetValue("sessionId", out var sessionIds);
            if (!success)
            {
